...
$servicesQuery = $connect->query("SELECT DISTINCT appointservice FROM appointments");
$serviceOptions = [];
while ($srv = $servicesQuery->fetch_assoc()) {
    $serviceOptions[] = $srv['appointservice'];
}
?>
<!DOCTYPE html>
<html lang="en">
            ...
            <main class="flex-1 p-6 md:ml-64 overflow-y-auto w-full min-h-screen bg-gray-50">
                ...
                <form action="" method="get">
                    <div
                        ...
                        <div>
                            <label class="text-sm text-gray-600 block mb-1">Search (Client Name)</label>
                    ...
                    <div class="flex justify-end mb-6">
                        <button type="submit"
                            class="bg-pink-600 text-white px-4 py-2 rounded hover:bg-pink-700 text-sm">Apply
                            Filters</button>
                    </div>
                </form>
                ...
                if (!empty($_GET['appointdate'])) {
                    ...
                    $condition .= " AND appointments.appointdate = '$formatted_date'";
                }
                if (!empty($_GET['services'])) {
                    $service = mysqli_real_escape_string($connect, $_GET['services']);
                    $condition .= " AND appointments.appointservice = '$service'";
                }
                if (!empty($_GET['status'])) {
                    $status = mysqli_real_escape_string($connect, $_GET['status']);
                    $condition .= " AND appointments.status = '$status'";
                }
                ...
                $query = "SELECT appointments.*, users.name AS username FROM appointments JOIN users ON appointments.userid = users.id $condition ORDER BY appointments.id DESC";
